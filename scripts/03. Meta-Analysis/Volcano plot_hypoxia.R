...
# Load data
meta_result <- read.csv("outputs/Meta-Analysis/Meta_combined_hypoxia_studies.csv",
                        na.strings = "")
...
top_genes <- meta_result |> mutate(Gene_Symbol = na_if(Gene_Symbol, "")) |> 
  ...
  slice_max(order_by = abs(log2FC), n = 50)
...
ggsave(filename = ("figures/Meta-Analysis/volcano plot_hypoxia.png"), 
       plot = volcano, 
       width = 8, 
       height = 8, 
       dpi = 600,
